...
public class EffectiveMetamodelComputationVisitor implements IEvlVisitor {
	...
	public void visit(OperationCallExpression operationCallExpression) {
		...
		if (!operationCallExpression.isContextless()) {
			...
			if (operationCallExpression.getTargetExpression().getResolvedType() instanceof EolModelElementType) {
				...
				target = (EolModelElementType) operationCallExpression.getTargetExpression().getResolvedType();
	...
	public void visit(PropertyCallExpression propertyCallExpression) {
		...
		EolType resolvedType = propertyCallExpression.getTargetExpression().getResolvedType();
	...
	public void visit(ConstraintContext constraintContext) {
		...
		if (constraintContext.getTypeExpression() != null) {
			target = (EolModelElementType) constraintContext.getTypeExpression().getResolvedType();
